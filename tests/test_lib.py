...
class TestLibSearch:
    def test_search(self):
        ...
        result = asyncio.run(onemap_sg.search("Chinese Garden MRT"))
...
class TestLibRouting:
    def test_walk(self):
        ...
        result = asyncio.run(
            onemap_sg.route_walk_drive_cycle(
                1.34235, 103.73260, 1.342, 103.743, "walk"
            )
        )
        assert result["route_summary"]["total_distance"] > 0

    def test_transit_rfc3339(self):
        import onemap_sg

        result = asyncio.run(
            onemap_sg.route_public_transport(
    ...
    def test_transit_timestamp(self):
        import onemap_sg
    ...
    def test_transit_full_rfc3339(self):
        import onemap_sg
...
class TestLibGeocode:
    def test_reverse_wgs84(self):
        import onemap_sg

        result = asyncio.run(onemap_sg.reverse_geocode_wgs84(1.34235, 103.73260))
...
class TestLibPopulation:
    def test_age_group(self):
        ...
        result = asyncio.run(onemap_sg.get_population_age_group("Bedok", 2020))
        # Response structure varies, just check it's a dict
...
class TestLibCoordinate:
    ...
    def test_convert(self):
        ...
        result = asyncio.run(onemap_sg.convert_4326_to_3414(1.34235, 103.73260))
...
class TestLibElevation:
    ...
    def test_point(self):
        ...
        result = asyncio.run(onemap_sg.get_elevation(1.352, 103.820))
...
class TestLibThemes:
    ...
    def test_list(self):
        ...
        result = asyncio.run(onemap_sg.get_all_themes_info())
